...
engine = create_async_engine('postgresql+asyncpg://postgres:fuckmetal1@localhost:5432/monitor', echo=True)
# engine = create_async_engine('postgresql+asyncpg://nemir4uk:0RxJiyA)Pa3YTz9P+v9XnXX@192.168.1.10:5532/instplatform', echo=True)
...
async def insert_objects(async_session: async_sessionmaker[AsyncSession]) -> None:
    async with async_session() as session:
        ...
        # print(log)
            ...
            # # await session.execute(stmt7)
            # # res = await session.execute(stmt8)
            # res2 = await session.execute(stmt11)
            # log = res2.fetchall()
            # # print(log)
            # for i in log:
            #     if i[0]['var_id']==1:
            #         for s in i[0]['slides']:
            #             print(s['question'])
            # # log_dct = [i._asdict() for i in log]
            # # # print(inst)
            # # for i in log:
            # #     if i[0]['var_id'] == 1:
            # #         for s in i[0]['slides']:
            ...
            # #             print(len(s['answers']))
            #
            # # r2 = await session.execute(stmt10)
            # # print(r2.fetchall())
